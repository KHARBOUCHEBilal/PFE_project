...
@section('content')
    <div class="page" id="page">
        <div class=new-admin id="new-admin">
            <div class="page-top">
                <div class="title">{{ __('pages/admin/global.edit') }}{{ __('pages/admin/global.subject') }}</div>
            ...
            <div class="page-body" id="page-body">
                ...
                <div class="body-content" id="body-content">
                    <form class="form" action="{{ route('subjects.update', $subject->id) }}" method="POST">
                        @csrf
                                    ...
                                    <select id="category_id" name="category_id">
                                        @if ($categories && $categories->count() > 0)
                                            <option value="" selected disabled>
                                                {{ __('pages/admin/global.choose_category') }}
                                            </option>
                                            <option value="" selected>Autre</option>

                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}"
                                                    @if ($subject->category_id == $category->id) checked @endif>{{ $category->name }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
                            ...
                            <div class="col-sm-12">
                                ...
                                <div class="action" style="padding-left: 30px">
                                    <label class="switch" type="button" data-toggle="modal" data-target="students-modal"
                                        data-keyboard='false'>
                                        <input type="checkbox" name="depend_on_s4" @if ($subject->depend_on_s4 == 'on') checked @endif>
